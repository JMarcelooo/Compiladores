...
fn main() {
    let buscarDadosUsuario = rt::new_func(move |args: Vec<JsVar>| -> JsVar {
    let userId = args.get(0).unwrap_or(&rt::new_undefined()).clone();
    rt::log(&[&rt::new_str(&format!("Buscando dados para o ID {}...", &userId.clone().borrow()))]);
    return /* new */ rt::Promise(&rt::new_func(move |args: Vec<JsVar>| -> JsVar {
    let resolve = args.get(0).unwrap_or(&rt::new_undefined()).clone();
        rt::setTimeout(&rt::new_func(move |args: Vec<JsVar>| -> JsVar {
    
            let dados = rt::new_object(vec![("1".to_string(), rt::new_object(vec![("nome".to_string(), rt::new_str("Alice")), ("status".to_string(), rt::new_str("Ativa")), ("ultimoLogin".to_string(), rt::new_str("2025-11-20"))])), ("2".to_string(), rt::new_object(vec![("nome".to_string(), rt::new_str("Bob")), ("status".to_string(), rt::new_str("Inativo")), ("ultimoLogin".to_string(), rt::new_str("2025-05-15"))])), ("3".to_string(), rt::new_object(vec![("nome".to_string(), rt::new_str("Charlie")), ("status".to_string(), rt::new_str("Ativa")), ("ultimoLogin".to_string(), rt::new_str("2025-11-24"))]))]);
            if rt::is_truthy(&rt::new_undefined()) {
                rt::call(&resolve.clone(), vec![rt::new_undefined()]);
            } else {
                rt::call(&resolve.clone(), vec![rt::new_object(vec![("nome".to_string(), rt::new_str("Usuário Não Encontrado")), ("status".to_string(), rt::new_str("Erro"))])]);
            }
        rt::new_undefined()
}), &rt::new_num(1000.0));
    rt::new_undefined()
    ...
    let carregarRelatorio = rt::new_func(move |args: Vec<JsVar>| -> JsVar {
    
    rt::log(&[&rt::new_str("Iniciando carregamento do relatório...")]);
    {
        let usuario1 = /* TODO: AwaitExpression */ rt::new_undefined();
        rt::log(&[&rt::new_str("\n✅ Dados do Usuário 1 (Alice) carregados:")]);
        rt::log(&[&usuario1.clone()]);
        let usuario2 = /* TODO: AwaitExpression */ rt::new_undefined();
        rt::log(&[&rt::new_str("\n✅ Dados do Usuário 2 (Bob) carregados:")]);
        rt::log(&[&usuario2.clone()]);
    }

    if false {
        let error = rt::new_str("Error");
{
        rt::log(&[&rt::new_str("❌ Ocorreu um erro no carregamento:"), &error.clone()]);
    }
    }
    rt::log(&[&rt::new_str("
...
rt::new_undefined()
    ...
    rt::call(&carregarRelatorio.clone(), vec![]);
}
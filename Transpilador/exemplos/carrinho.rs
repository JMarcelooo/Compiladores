...
fn main() {
    let calcularTotal = rt::new_func(move |args: Vec<JsVar>| -> JsVar {
    let itens = args.get(0).unwrap_or(&rt::new_undefined()).clone();
    let percentualDesconto = args.get(1).unwrap_or(&rt::new_undefined()).clone();
    let subtotal = rt::new_num(0.0);
    rt::for_each_variant(&itens.clone(), |args: Vec<JsVar>| -> JsVar {
    let item = args.get(0).unwrap_or(&rt::new_undefined()).clone();
        let valorItem = rt::mul(&rt::get(&item.clone(), "preco"), &rt::get(&item.clone(), "quantidade"));
        rt::replace(&subtotal, &rt::add(&subtotal, &valorItem.clone()));
    rt::new_undefined()
});
    let valorDesconto = rt::mul(&subtotal.clone(), &percentualDesconto.clone());
    let totalFinal = rt::sub(&subtotal.clone(), &valorDesconto.clone());
    return rt::new_object(vec![("subtotal".to_string(), subtotal.clone()), ("descontoAplicado".to_string(), valorDesconto.clone()), ("totalFinal".to_string(), totalFinal.clone())]);
});
    let carrinho = rt::new_array(vec![rt::new_object(vec![("nome".to_string(), rt::new_str("Notebook Dell")), ("preco".to_string(), rt::new_num(4500.0)), ("quantidade".to_string(), rt::new_num(1.0))]), rt::new_object(vec![("nome".to_string(), rt::new_str("Mouse sem fio")), ("preco".to_string(), rt::new_num(79.9)), ("quantidade".to_string(), rt::new_num(2.0))]), rt::new_object(vec![("nome".to_string(), rt::new_str("Teclado Mecânico")), ("preco".to_string(), rt::new_num(250.5)), ("quantidade".to_string(), rt::new_num(1.0))]), rt::new_object(vec![("nome".to_string(), rt::new_str("Monitor 27''")), ("preco".to_string(), rt::new_num(1200.0)), ("quantidade".to_string(), rt::new_num(1.0))])]);
    let taxaDesconto = rt::new_num(0.1);
    let resumo = rt::call(&calcularTotal.clone(), vec![carrinho.clone(), taxaDesconto.clone()]);
    rt::log(&[&rt::new_str("=== Resumo do Carrinho ===")]);
    rt::log(&[&rt::new_str(&format!("Subtotal dos Itens: R$ {}", &rt::to_fixed(&rt::get(&resumo.clone(), "subtotal"), &rt::new_num(2.0)).borrow()))]);
    rt::log(&[&rt::new_str(&format!("Desconto ({}%): R$ {}", &rt::to_fixed(&rt::mul(&taxaDesconto.clone(), &rt::new_num(100.0)), &rt::new_num(0.0)).borrow(), &rt::to_fixed(&rt::get(&resumo.clone(), "descontoAplicado"), &rt::new_num(2.0)).borrow()))]);
    rt::log(&[&rt::new_str("---------------------------")]);
    rt::log(&[&rt::new_str(&format!("Total a Pagar: R$ {}", &rt::to_fixed(&rt::get(&resumo.clone(), "totalFinal"), &rt::new_num(2.0)).borrow()))]);
}
/**
 * transpilador_v3.js (Versão V13 - For Loop Added)
 * * Adiciona: Suporte ao loop clássico 'for (init; test; update)'.
 * * Mantém: Todas as correções anteriores (V12 Hybrid).
 */
...
class RustRuntimeTranspiler {
    ...
    visit(node) {
        ...
        switch (node.type) {
            ...
            case 'ForStatement': return this.visitForStatement(node); // NOVO
...
${handlerBody}
    ...
    // NOVO: Suporte ao loop for (C-style)
    visitForStatement(node) {
        ...
        // body já vem como bloco { ... }
        const bodyContent = this.visit(node.body);
        
        // Precisamos injetar o update no final do bloco do while
        // Como visitBlockStatement retorna "{\n ... \n}", inserimos antes do último }
        let loopBody = bodyContent;
        if (update) {
            // Remove a chave final para inserir o update
            if (loopBody.trim().endsWith('}')) {
                ...
                // Se for single statement, envolve em bloco
                loopBody = `{\n${loopBody}\n${update}\n}`;
            }
        }
        ...
        {
            ${init}
            while rt::is_truthy(&${test}) ${loopBody}
        }`;
    }
...
function main() {
    ...
    try {
        const jsCode = fs.readFileSync(args[0], 'utf8');
        ...
        fs.writeFileSync(args[1], rustCode);
        console.log(`✅ Arquivo Rust V13 (For Loop) gerado em: ${args[1]}`);
    } catch (err) {
